...
def google_gemini_api_key() -> str | None:
    """First non-empty Gemini / Google AI Studio key from supported env names."""
...
def _pick_ai_commentary_backend():
    ...
    if prov in ("off", "disabled", "none", "no", "false", "0"):
        return None
    if prov == "openai":
        order = ["openai"]
    elif prov == "anthropic":
    ...
    for p in order:
        ...
        if p == "google" and google_gemini_api_key():
    ...
    if picked is None:
        return json_response(
            ...
            503,
            {
                ...
                "hint": "Optional: AI_COMMENTARY_PROVIDER=openai|anthropic|google|xai|auto|off (default auto picks first available; off disables AI).",
            },
        )
    if not quote_in:
        try:
                ...
                200,
                {
                    ...
                    "llm_commentary": {
                        ...
                        "configured": {
                            ...
                            "google": bool(google_gemini_api_key()),
...
def main() -> int:
    ...
    print("Health check: GET /api/health  →  expect api_revision: 21, llm_commentary, shared_family_portfolio.", flush=True)
